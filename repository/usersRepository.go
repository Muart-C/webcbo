...
func CreateUser(userName, email, firstName, lastName, password string) (*models.User, error) {
...
func FetchUsers(offset,limit int) (*[]models.User,int, error) {
	var users []models.User
	models.DB.Find(&users).Offset(offset).Limit(limit)
	if len(users) > 0 {
		return &users,len(users), nil
	}
	return nil,0, errors.New("you have no user records")
}
